...
object SG_Cov {
  ...
  def main(args: Array[String]) {
    ...
    if (args.length != 5) {
      println("args: <K> <BatchNum> <ALLGain> <input> <ouput>")
      return
    }
    val conf = new SparkConf().setAppName("StreamGreedy")
    ...
    while (i < B) { //total batch = B
      ...
      val BatchData = sc.textFile(inputPath + i).map(StringToBatchObject_Cov)
      ...
      //3.Output Result
      res = "StreamGreedy algorithm Coverage : " + Gain.GOS(sumZ) * 100 / AllGain + "% \n";
    ...
    //3.Output Result
    sc.parallelize(res).saveAsTextFile(outputPath + "StreamG" + SaveStamp)
